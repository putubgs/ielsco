...
type GIFRegistration = {
  ...
  is_lounge_member: boolean; // Udah diganti ke boolean sesuai Supabase
...
export default function GIFDashboardPage() {
    ...
    const init = async () => {
      try {
        ...
        if (!data) {
          ...
          if (insertError) {
             console.error("GAGAL INSERT KE SUPABASE:", JSON.stringify(insertError, null, 2));
             console.error("Pesan Error:", insertError.message);
          } else {
             setRegData(newData);
          }
        } else {
  ...
  const handlePhase1Click = async () => {
    ...
    // Opsional: Langsung update secara lokal buat visual feedback (walau disync sama Apps Script nantinya)
  ...
  // Hitung step yang udah selesai. Kalau allRequirementsMet, langsung 3.
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto pb-20 space-y-8 px-4 md:px-8 pt-8 font-geologica">
        ...
        {/* === WHATSAPP COMMUNITY BANNER (IELS COLOR PALETTE) === */}
        <motion.div 
        ...
        <div className="bg-white rounded-2xl border border-[#304156]/10 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm text-gray-500">
              {completedStepsCount} of 3 steps completed
            </span>
          </div>
            ...
            <div className="relative flex justify-between items-start">
              ...
              <div className="flex flex-col items-center text-center w-24">
                <div className={cn(
                  ...
                  (allRequirementsMet || regData?.phase1_status === 'submitted') ? "bg-[#914D4D]" : "bg-gray-200"
                )}>
                  {(allRequirementsMet || regData?.phase1_status === 'submitted') ? (
              ...
              <div className="flex flex-col items-center text-center w-24">
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-lg mb-2 transition-all",
                  (allRequirementsMet || regData?.phase2_status === 'submitted') ? "bg-[#914D4D]" : "bg-gray-200"
                )}>
                  {(allRequirementsMet || regData?.phase2_status === 'submitted') ? (
                    <CheckCircle className="w-5 h-5 text-white" />
                  ) : (
              ...
              <div className="flex flex-col items-center text-center w-24">
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-lg mb-2 transition-all",
                  isLoungeDone ? "bg-[#914D4D]" : "bg-gray-200"
                )}>
                  {isLoungeDone ? (
                    <CheckCircle className="w-5 h-5 text-white" />
                  ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              {/* Price Banner (Hanya muncul jika BELUM enroll/mentoring) */}
              {!isLoungeDone && (
        ...
        {allRequirementsMet ? (
          /* ACTIVE MENTORING DASHBOARD ACCESS - REVISED COLORS */
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
